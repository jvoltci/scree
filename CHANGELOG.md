...
- **Triton autograd wrapper.** `scree.kernels.triton.varlen_attention_triton_autograd` wraps the Triton forward kernel in a `torch.autograd.Function` so users can backprop through it. Forward uses the fast Triton kernel; backward uses the reference (slow Python loop) implementation under `torch.enable_grad`. Verified on H100: forward correctness PASS (max abs diff 4.88e-04 vs FA-2), backward correctness PASS (dq/dk/dv diffs 2-4e-03 vs FA-2). Full training-step timing: 15.18 ms scree vs 0.69 ms FA-2 (~22× slower, dominated by the reference backward). Triton backward kernel is v0.1 work.
